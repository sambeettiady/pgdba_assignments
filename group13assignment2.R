...
#Accuracy = 62%
set.seed(37)
lm_ctrl = trainControl(method = 'cv',number = 10,search = 'grid')
lambda.grid = 10*seq(-2,2,length=100)
alpha.grid = seq(0,1,length = 20)
search_grid = expand.grid(.alpha = alpha.grid,.lambda = lambda.grid)
lmp = train(data = train_outliers_removed,quality ~ volatileacidity + chlorides + trappedsulphurdioxide + 
                sulphates_log + alcohol,method = 'glmnet',trControl = lm_ctrl,tuneGrid = search_grid)

test$pred_enet = predict(object = lmp,newdata = test)

test$pred_lm = predict(object = lmfit2,newdata = test)
